...
@info "Servidor carregado" Threads.nthreads()
...
function parse_investing_csv_text(filename::AbstractString, content::AbstractString)
    ...
    for c in [:date_str, :last_str, :open_str, :high_str, :low_str, :vol_str, :var_str]
...
function build_long_from_investing_payload(payload)
    ...
    bench = all_df[
        all_df.asset .== benchmark_asset,
        [:datetime, :asset_open, :asset_high, :asset_low, :asset_close]
    ]
    ...
    if any(ismissing.(panel.bench_open)) ||
       any(ismissing.(panel.bench_high)) ||
       any(ismissing.(panel.bench_low)) ||
       any(ismissing.(panel.bench_close))
...
function safe_float(x)
    ...
    elseif x isa AbstractString
        t = strip(String(x))
        return isempty(t) ? NaN : Float64(parse(Float64, t))
    else
        return Float64(x)
    end
end
...
function forward_backtest(panel::DataFrame, feature_cols::Vector{Symbol}, params)
    ...
    for d in test_dates
        ...
        day_df = panel[panel.datetime .== d, :]
        score_df = day_df[finite_mask(day_df, feature_cols; need_target=false), :]
...
const openApi = OpenAPI("3.0.0", info)
const swagger_document = build(openApi)
